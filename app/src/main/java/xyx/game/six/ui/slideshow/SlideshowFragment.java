...
//                        a[7][1],a[7][2],a[7][3],a[7][4],a[7][5],a[7][6])),stoi(getstr(a,0)));
    ...
    private ArrayList<Integer>  threekill(int a[][],int b){
        ...
        ArrayList<Integer> a1=first(a,b);
    ...
    private String getResult(){
        ...
        str=getstr(a);
        for (int i = 0; i <10; i++) {
            str+="\n"+(i+8)+"期预测"+first(a,i).toString()+"="+first(a,i).size();

            //str+=then1(a,i);
            // str+=then2(a,i);
        }

        str+="\n"+(18)+"期预测"+first(a,10).toString()+"="+first(a,10).size();
        for (int i = 0; i <10; i++) {

            str+="\n"+(i+8)+"期测中"+zhongjian(new ArrayList<Integer>(Arrays.asList(a[i+7][0],a[i+7][1],
                    a[i+7][2],a[i+7][3],a[i+7][4],a[i+7][5],a[i+7][6])),first(a,i))
                    +sayResult(first(a,i),a[i+7][6])+a[i+7][6];
            //str+=then1(a,i);
            // str+=then2(a,i);
        }
    ...
    //计算
    private ArrayList<Integer>  first(int [][] a,int b){
    ...
    private ArrayList<Integer>  kill0(int [][] a,int b){
        ...
        for (int i = 1; i <=49; i++) {
            //System.out.println((i+1)+"="+ Collections.frequency(list, i+1));
            //统计出现2次的数值
            if (Collections.frequency(list, i)==2){list2.add(i); }
        }
        return list2;
